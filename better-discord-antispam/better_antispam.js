...
module.exports = async (client, options) => {
  ...
  const timeMuted = (options && options.timeMuted) || 10000; // Default value: 10 minutes. Explication: This is how much time member X will be muted. if not set, default would be 10 min.
 ...
 client.on("checkMessage", async (message) => {
  ...
  const MuteMember = async (m, muteMsg) => {
    ...
    for (var i = 0; i < messageLog.length; i++) {
      ...
      //user.get("slavce#1172").send("Mutiran je user")
      //author.find("slavce#1172").sendMessage(`"Mutiran je korisnik"`)
      ...
      if (user) {
        user.roles.add(role).then(()=>{
          ...
          let muteEmbed = new MessageEmbed()
            ...
            .addField(`Koliko je mutiran?:`,`${timeMuted} seconds (10 min)`)
            .addField('Zasto?: ', `Spam`)
            .addField(`Do kad je mutiran:`,TheDate+ " at "+ clock+" "+amORpm)
            .setColor('#D9D900')
